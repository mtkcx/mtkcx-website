-- CRITICAL SECURITY FIX: Address data exposure vulnerabilities (Fixed version)
...
DROP POLICY IF EXISTS "Only admins can manage all conversations" ON public.chat_conversations;

-- Ensure only proper access (recreate with proper names)
CREATE POLICY "Secure user conversation access" 
ON public.chat_conversations 
FOR SELECT 
TO authenticated
USING (auth.uid() = user_id OR is_admin());

CREATE POLICY "Secure admin conversation management" 
ON public.chat_conversations 
FOR ALL 
TO authenticated
USING (is_admin())
...
DROP POLICY IF EXISTS "Users can view messages in their conversations only" ON public.chat_messages;

CREATE POLICY "Secure user message access" 
...
DROP POLICY IF EXISTS "Only admins can view enrollment requests" ON public.enrollment_requests;
DROP POLICY IF EXISTS "Only admins can manage enrollment requests" ON public.enrollment_requests;

-- Ensure only admins can access enrollment data (new policy names)
CREATE POLICY "Secure admin enrollment view access" 
ON public.enrollment_requests 
FOR SELECT 
TO authenticated
USING (is_admin());

CREATE POLICY "Secure admin enrollment management" 
ON public.enrollment_requests 
FOR ALL 
TO authenticated
USING (is_admin())
...
DROP POLICY IF EXISTS "Secure newsletter signup via edge function" ON public.newsletter_subscriptions;

-- Ensure only admins can access subscriber data
CREATE POLICY "Secure admin newsletter view access" 
ON public.newsletter_subscriptions 
FOR SELECT 
TO authenticated
USING (is_admin());

CREATE POLICY "Secure admin newsletter management" 
ON public.newsletter_subscriptions 
FOR ALL 
TO authenticated
USING (is_admin())
...
CREATE POLICY "Secure newsletter registration via edge function" 
ON public.newsletter_subscriptions 
FOR INSERT 
TO anon
WITH CHECK (
...
-- Ensure users can only access their own profiles (recreate with new names)
CREATE POLICY "Secure user profile view access" 
ON public.profiles 
FOR SELECT 
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Secure user profile update access" 
ON public.profiles 
FOR UPDATE 
TO authenticated
USING (auth.uid() = user_id)
...
CREATE POLICY "Secure user profile creation access" 
ON public.profiles 
FOR INSERT 
TO authenticated
WITH CHECK (auth.uid() = user_id);
