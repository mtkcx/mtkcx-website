  ...
  return <section className="py-12 sm:py-16 lg:py-20 bg-background">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          return <Card key={index} className="relative group hover:shadow-lg transition-all duration-300 border-2 hover:border-primary/20 will-change-transform hover:scale-105 h-full flex flex-col">
                ...
                <CardContent className="space-y-4 sm:space-y-6 px-4 sm:px-6 flex-1 flex flex-col">
                  <ul className="space-y-2 sm:space-y-3 flex-1">
                     {service.features.map((feature, featureIndex) => <li key={`feature-${service.title}-${featureIndex}-${feature.substring(0,10)}`} className="flex items-center text-sm sm:text-base text-muted-foreground">
